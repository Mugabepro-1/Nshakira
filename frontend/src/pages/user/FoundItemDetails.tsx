...
import { PackageOpen, Calendar, MapPin, User, Info, Phone } from 'lucide-react';
...
const FoundItemDetails = () => {
  ...
  useEffect(() => {
    const fetchItemDetails = async () => {
      ...
      setLoading(true);
      try {
        const data = await itemsService.getFoundItemById(id);
        setItem(data);
      } catch (error) {
  ...
  const handleClaim = async (values: { description: string }) => {
    ...
    try {
      await claimsService.claimFoundItem(id, { description: values.description });
      toast.success('Claim submitted successfully');
      setShowClaimForm(false);
      // Refresh item details to show claim status
      const updatedItem = await itemsService.getFoundItemById(id);
      setItem(updatedItem);
    } catch (error) {
  ...
  if (!item) {
    return (
      <div className="text-center py-12">
        ...
        <Button as={Link} to="/found" variant="primary" className="mt-4">
          Back to Found Items
        </Button>
      </div>
    );
  ...
  const formattedDate = item.foundDate 
    ? format(new Date(item.foundDate), 'MMMM dd, yyyy')
    : 'Unknown date';
    ...
    <div className="animate-fade-in">
      <div className="mb-6">
        <div className="flex items-center">
          <Link to="/found" className="text-secondary-600 hover:text-secondary-700 mr-2">
            Found Items
          </Link>
        ...
        <div>
          <Card title="Item Details">
            <div className="space-y-4">
              ...
              <div className="flex items-start">
                <PackageOpen className="h-5 w-5 text-gray-400 mt-0.5 mr-2" />
                <div>
