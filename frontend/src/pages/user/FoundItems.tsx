  ...
  useEffect(() => {
    const fetchFoundItems = async () => {
      ...
      try {
        ...
        const response = await itemsService.getFoundItems(params);
        const data = response as FoundItemsResponse;
    ...
    <div className="animate-fade-in">
      <div className="mb-6">
        ...
        <p className="mt-1 text-gray-600">
          Browse all reported found items. If you've lost any of these items, you can claim them.
        </p>
      </div>

      <div className="mb-6 flex flex-col space-y-4 md:flex-row md:justify-between md:space-y-0">
        <SearchInput onSearch={handleSearch} placeholder="Search found items..." />
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {foundItems.map((item) => (
              <Link key={item.id} to={`/found/${item.id}`}>
                <Card hoverable className="h-full transition-all duration-200 hover:translate-y-[-4px]">
                  <div className="relative aspect-[4/3] w-full overflow-hidden rounded-md bg-gray-200 mb-4">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent px-4 py-2">
                      <span className="inline-block rounded-full bg-secondary-100 px-2 py-1 text-xs font-semibold text-secondary-800">
                        {item.category}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    ...
                    <div className="flex items-center">
                      ...
                      <span>
                        {format(new Date(item.foundDate), 'MMM dd, yyyy')}
                      </span>
                    </div>
                  </div>
                </Card>
